...
belonging <- belonging %>% mutate(
  ...
  `Cultural / Racial Background` = SDCDGCGT,
...
belonging <- belonging %>% filter( 
  !`Cultural / Racial Background` %in% c(6,9)) #n = , t = 
belonging$`Cultural / Racial Background` <- factor(belonging$`Cultural / Racial Background`,
                             levels = c(1,2),
                             labels = c("White", 
...
log_model_belonging <- svyglm(`Illicit Drug Use - past 12 months` ~ `Social Integration` + `Province` + `Gender` + `Age Group` + `Highest Level Of Education` + `Total Household Income` + `Cultural / Racial Background`,
                                   data = belonging, 
