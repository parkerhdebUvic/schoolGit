...
path <- paste0(here(), "/data/COVID19 Long-Term Care Outbreak Dataset_R.xlsx")

linelist <-    import(path)                           ########## import data ##########
...
linelist_clean <- linelist %>% 
  ...
  rename(onset_date = date_onset, unique_id = caseid)  %>%                                 ########## rename caseid to unique_id ##########
...
linelist_clean %>% 
  ...
  summarise(
    ...
    n_females = sum(sex == "Female", na.rm=T),                                     ########## add code to summarize female cases ##########
    n_hospitalized = sum(hospitalized == "Yes", na.rm = T),                                            ########## add code to summarize hospitalized ##########
...
linelist_clean %>% 
  ...
  summarise(
    ...
    "Number of Females" = sum(sex == "Female", na.rm=T),                                            ########## add code to summarize female cases ##########
    "Number of cases hospitalized" = sum(hospitalized == "Yes", na.rm=T),                                            ########## add code to summarize hospitalized ##########
