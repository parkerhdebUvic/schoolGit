...
class BinarySearchTree <K extends Comparable<K>, V>  {
	...
	private V find(BSTNode<K,V> cur, K key) throws KeyNotFoundException {
		if (cur == null) {
			throw new KeyNotFoundException();
		}
		findLoops++;
	...
	public int height(BSTNode<K,V> cur) {
		if (cur == null) {
			return -1;
		} else {
	...
	// completed for you
	private void inOrderRecursive (BSTNode<K,V> n, List <Entry<K,V>> resultList) {
		if (n == null) {
			return;
		}
	...
	// completed for you
	private void preOrderRecursive (BSTNode<K,V> n, List <Entry<K,V>> resultList) {
		if (n == null) {
			return;
		}
		resultList.add(new Entry<K,V>(n.key, n.value));
	...
	// completed for you
	private void postOrderRecursive (BSTNode<K,V> n, List <Entry<K,V>> resultList) {
		if (n == null) {
			return;
		}
