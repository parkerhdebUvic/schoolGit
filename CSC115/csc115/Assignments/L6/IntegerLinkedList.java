public class IntegerLinkedList {
	...
	private int productHelper(IntegerNode n){
		if (n == null){
			...
			int first = n.getData();
			int prodRest = productHelper(n.next);

			return first * prodRest;
		}
	}
	...
	private boolean allNegativeHelper(IntegerNode n){
		if (n == null){
			...
			return (n.getData() < 0) && allNegativeHelper(n.next);
		}
	}
	...
	private boolean isSortedAscendingHelper(IntegerNode n, int val){
		if (n == null){
			return true;
		} else {
			return (val <= n.getData()) && isSortedAscendingHelper(n.next, n.getData());
		}
	}
