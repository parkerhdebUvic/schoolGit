...
class BinarySearchTree <K extends Comparable<K>, V>  {
	...
	public void insert (K key, V value) {
		...
		if (root==null){
			...
			insert(null, root, key, value);
		}
	}

	private void insert(BSTNode<K,V> parent, BSTNode<K,V> cur, K key, V val){
		if (cur!=null){
			if (cur.getKey().compareTo(key) == 0){
				cur.setValue(val);
			} else if (cur.getKey().compareTo(key) > 0){
				insert(cur, cur.getLeft(), key, val);
			} else {
				insert(cur, cur.getRight(), key, val);
			}
		} else {
			if (parent.getKey().compareTo(key) > 0){
				parent.setLeft(key, val);
				count++;
			} else{
				parent.setRight(key,val);
				count++;
			}
		}
	}
	...
	public V find(BSTNode<K,V> t, K key) throws KeyNotFoundException{
		if (t==null){
			throw new KeyNotFoundException();
		} else {
	...
	public int height() {
		// TODO: implement this
		return height(root); // so it compiles
	}
	...
	/* 
	 * Purpose: Return a list of all the key-value Entry elements (BSTNode's?) 
	...
	private void inOrderRec (BSTNode<K,V> n, List <Entry<K,V>> entries) {
		...
		if(n==null){
			...
			inOrderRec(n.getLeft(), entries);
			//add to list
			entries.add(new Entry<K,V>(n.key, n.value));
			inOrderRec(n.getRight(), entries);
		}
	}
	...
	private void preOrderRec (BSTNode<K,V> n, List <Entry<K,V>> entries) {
		...
		if (n==null){
			...
			//add entry to list
			entries.add(new Entry<K,V>(n.key, n.value));
			preOrderRec(n.getLeft(), entries);
			preOrderRec(n.getRight(), entries);
		}
	}
	...
	private void postOrderRec (BSTNode<K,V> n, List <Entry<K,V>> entries) {
		...
		if (n==null){
			...
			postOrderRec(n.getLeft(), entries);
			postOrderRec(n.getRight(), entries);
			//add to list
			entries.add(new Entry<K,V>(n.key, n.value));
		}
	}
